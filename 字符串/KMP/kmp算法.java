...
class KMP {
    ...
    private int kmp(String s, String p) {
        ...
        return j == n ? i - n : -1; // 匹配成功则返回起始下标，否则返回-1
    }

    private int[] getNext(String p) {
        int n = p.length();
        // next[j]: 子串p[0...j-1]的"相等前后缀"的最大长度
        ...
        for (int j = 1; j <= n; j++) {
            int cur = next[j - 1];
            while (cur != -1 && p.charAt(cur) != p.charAt(j - 1)) {
                cur = next[cur];
                // 注意: 若统计这里的while循环次数, 会发现while循环总次数(整个外层for循环)一定是n-1，故整体时间复杂度为O(n)
            }
            next[j] = cur + 1;
        }
        return next;
